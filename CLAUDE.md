...
## Security — download confinement (2026-07)

Audit ref `agent-runtime/_code_review/02-security-audit.md` (C6/H10/M11),
mitigations `_mitigation/09`. The bucket-allowlist chokepoint was always solid;
the host-write side is now confined too:
- ✅ **`gcs_download` writes ONLY under `download_dir`** (config key; default
  `<arc_home>/downloads`). `local_path` is resolved relative to it; absolute
  paths and `..`/symlink escapes are rejected (`_confine_download`). `~` is NOT
  expanded (stays a literal subdir).
